...
namespace Koturn.VRChat.Log
{
    ...
    public abstract class VRCBaseLogParser : IDisposable
    {
        ...
        /// <summary>
        /// Byte sequence of "Log       ".
        /// </summary>
        [Obsolete]
        private static readonly byte[] LogLogSequence = new[]
        {
            (byte)'L', (byte)'o', (byte)'g', CodeSp, CodeSp, CodeSp, CodeSp, CodeSp, CodeSp, CodeSp
        };
        /// <summary>
            ...
            do
            {
                ...
                else
                {
                    ...
                    unsafe
                    {
                        ...
                        {
                            var firstLineMessage = ParseFirstLogLine(pBuffer, count, out DateTime logDateTime2, out LogLevel logLevel2);
        ...
        private unsafe string? ParseFirstLogLine(byte *pBuffer, int count, out DateTime logDateTime, out LogLevel logLevel)
        {
            ...
            else if (IsMatchSequence(pLogLevel, LogLogSequence))
            {
                logLevel = LogLevel.Debug;
            }
        ...
        {
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP2_1_OR_GREATER
            return new ReadOnlySpan<byte>(pBuffer, sequence.Length).SequenceEqual(sequence);
#else
